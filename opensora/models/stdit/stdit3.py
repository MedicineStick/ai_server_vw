...
class STDiT3(PreTrainedModel):
    ...
    def forward(self, x, timestep, y, mask=None, x_mask=None, fps=None, height=None, width=None, **kwargs):
        ...
        # === get timestep embed ===
        t = self.t_embedder(timestep.to(next(self.t_embedder.parameters()).device), dtype=x.dtype)  # [B, C]
        ...
        if x_mask is not None:
            ...
            t0 = self.t_embedder(t0_timestep.to(fps.device), dtype=x.dtype)
            t0 = t0 + fps
        ...
        if self.config.skip_y_embedder:
            ...
            y, y_lens = self.encode_text(y.to(fps.device), mask.to(fps.device))

        # === get x embed ===
        xdevice = x.device
        x = self.x_embedder(x.to(fps.device))  # [B, N, C]
        x = rearrange(x, "B (T S) C -> B T S C", T=T, S=S)
        x = x.to(xdevice) + pos_emb
        ...
        for spatial_block, temporal_block in zip(self.spatial_blocks, self.temporal_blocks):
            x = auto_grad_checkpoint(spatial_block, x.to(y.device), y, t_mlp, y_lens, x_mask.to(y.device), t0_mlp, T, S)
            x = auto_grad_checkpoint(temporal_block, x.to(y.device), y, t_mlp, y_lens, x_mask.to(y.device), t0_mlp, T, S)
        ...
        # === final layer ===
        x = self.final_layer(x, t, x_mask.to(x.device), t0, T, S)
