...
class STDiT2(PreTrainedModel):
    ...
    def forward(
        ...
        # embedding
        x_embedder_device = next(self.x_embedder.parameters()).device
        
        x = self.x_embedder(x.to(x_embedder_device))  # [B, N, C]
        x = rearrange(x, "B (T S) C -> B T S C", T=T, S=S)
        x = x + pos_emb.to(x_embedder_device)
        x = rearrange(x, "B T S C -> B (T S) C")

        # prepare adaIN
        t = self.t_embedder(timestep.to(x_embedder_device), dtype=x.dtype)  # [B, C]
        t_spc = t + data_info  # [B, C]
        t_tmp = t + fl  # [B, C]
        t_spc_mlp = self.t_block(t_spc)  # [B, 6*C]
        t_tmp_mlp = self.t_block_temp(t_tmp)  # [B, 3*C]
        if x_mask is not None:
            t0_timestep = torch.zeros_like(timestep).to(x_embedder_device)
        ...
        # prepare y
        y = self.y_embedder(y.to(x_embedder_device), self.training)  # [B, 1, N_token, C]

        if mask is not None:
            mask = mask.to(x_embedder_device)
        ...
        # final process
        x = self.final_layer(x, t, x_mask.to(x.device), t0_spc, T, S)  # [B, N, C=T_p * H_p * W_p * C_out]
